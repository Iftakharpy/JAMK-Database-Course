...
-- I also renamed the gname to firstname in an additional query for convenience.
SELECT gname, surname FROM emps WHERE gname LIKE 'MI%'; 
SELECT gname AS firstname, surname FROM emps WHERE gname LIKE 'MI%'; 
...
SELECT gname AS firstname, surname FROM emps WHERE empnum BETWEEN 50 AND 70; 
...
SELECT * FROM emps WHERE SUBSTRING(gname, 2, 1)='A' AND surname LIKE '%N' AND pc LIKE 'T0%';
